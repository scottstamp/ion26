...
namespace Ion.Net.Connections
{
    ...
    public class IonTcpConnection
    {
        ...
        private void ConnectionDead()
        {
            try
            {
                // Notify the users friends that it goes offline
                //foreach (MessengerBuddy buddy in IonEnvironment.GetHabboHotel().GetClients().GetClient(mID).GetMessenger().GetBuddies().Values)
                //{
                //    if (buddy.Online)
                //        IonEnvironment.GetHabboHotel().GetClients().GetClientOfHabbo(buddy.ID).GetMessenger().removeBuddy(IonEnvironment.GetHabboHotel().GetClients().GetClient(mID).GetHabbo().ID);
                //}
                //IonEnvironment.GetHabboHotel().GetClients().GetClient(mID).GetMessenger().notifyFriends(true);
            }
            catch { }
        ...
        {
            IonEnvironment.GetLog().WriteLine(" [" + mID + "] <-- " + message.Header + message.GetContentString().Replace(Convert.ToChar(13).ToString(), "{13}"));
