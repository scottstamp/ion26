            ...
            lock (this)
            {
                ...
                using (DatabaseClient dbClient = IonEnvironment.GetDatabase().GetClient())
                {
                    ...
                    if(index != null)
                    {
                        ...
                        foreach (DataRow childRow in dbClient.ReadDataTable("SELECT * FROM catalog_pages WHERE parentid = " + index.TreeID + " ORDER BY orderid ASC;").Rows)
                        {
                            ...
                            if (child != null)
                            {
                                ...
                                if (child.TreeID > 0)
                                {
                                    ...
                                    foreach (DataRow pageRow in dbClient.ReadDataTable("SELECT * FROM catalog_pages WHERE parentid = " + child.TreeID + " AND treeid = 0 ORDER BY orderid ASC;").Rows)
                                    {
