    ...
    public class GameClientManager
    {
        ...
        private void TestClientConnections()
        {
            ...
            while (true)
            {
                ...
                {
                    // Prepare PING data
                    ServerMessage pingdata = new ServerMessage(ResponseOpcodes.Ping); // "@r"
                    ...
                    lock (mClients)
                    {
                        try
                        {
                            foreach (GameClient client in mClients.Values)
                            {
                                ...
                                {
                                    client.PingOK = false;
                                    client.GetConnection().SendMessage(pingdata);
                                }
                                else
                                {
        ...
        public GameClient GetClientOfHabbo(string Username)
        {
            lock (mClients)
            {
                foreach (GameClient client in mClients.Values)
