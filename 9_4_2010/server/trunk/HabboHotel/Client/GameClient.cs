    ...
    public class GameClient
    {
        ...
        public void StartConnection()
        {
            if (mConnection != null)
            {
                ...
                // Send hello message
                mMessageHandler.GetResponse().Initialize(0); // "@@"
        ...
        private void CompleteLogin()
        {
            ...
            {
                // Send unknown packet
                mMessageHandler.GetResponse().Initialize(257); // "DA"
                mMessageHandler.GetResponse().Append("QBHIIIKHJIPAIQA" + "dd-MM-yyyy" + Convert.ToChar(2) + "SAHPB" + "/client" + Convert.ToChar(2) + "QBH"); // TODO: too lazy
                mMessageHandler.SendResponse();
                ...
                // Send another unknown packet
                mMessageHandler.GetResponse().Initialize(290); // "Db"
                mMessageHandler.GetResponse().AppendBoolean(false);
                mMessageHandler.GetResponse().AppendBoolean(false);
                mMessageHandler.SendResponse();
                ...
                // Check if welcome message is enabled, if so, send it
                ...
                {
                    dbClient.AddParamWithValue("@now", DateTime.Now.ToString("dd'-'MM'-'yyyy hh':'mm':'ss tt"));
                    dbClient.AddParamWithValue("@id", mHabbo.ID);
                    dbClient.ExecuteQuery("UPDATE users SET lastvisit = @now WHERE id = @id");
                }

                //// Start Activity Point giver thread
                //ThreadStart tStart = new ThreadStart(giveActivityPoints);
                //Thread T = new Thread(tStart);
                //T.Start();
            }
        }
        //private void giveActivityPoints()
        //{
        //    try
        //    {
        //        while (true)
        //        {
        //            AddActivityPoints(uint.Parse(IonEnvironment.GetConfiguration().GetValue("projects.habbo.users.amount_activitypoints")));
        //            Thread.Sleep(int.Parse(IonEnvironment.GetConfiguration().GetValue("projects.habbo.users.habbactivitypoints_interval_minutes")) * 60000);
        //        }
        //    }
        //    catch { return; }
        //}
