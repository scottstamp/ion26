...
namespace Ion.HabboHotel.Navigator
{
    public class NavigatorManager
    {
        ...
        public List<NavigatorRoom> getRecommendedRooms()
        {
            List<NavigatorRoom> mrooms = new List<NavigatorRoom>();
            List<NavigatorRoom> rooms = new List<NavigatorRoom>();
            foreach (NavigatorRoom room in mRooms.Values)
                mrooms.Add(room);
            for (int i = 0; i < 3; i++)
            {
                int random = new Random().Next(0, mRooms.Count);
                rooms.Add(mrooms[random]);
            }
            return rooms;
        }
        ...
        public uint getVisitors(uint categoryID)
        {
            uint visitors = 0;
            foreach (Room room in IonEnvironment.GetHabboHotel().GetRooms().getRooms(categoryID))
            {
                visitors += room.Visitors;
            }
            return visitors;
        }
        public uint getMaxVisitors(uint categoryID)
        {
            uint maxVisitors = 0;
            foreach (NavigatorRoom room in getRooms(categoryID))
            {
